...
class DTWAligner:
    ...
    def __init__(self, predict_list, target_dict, predict_dir, target_dir,args,hparams):
        ...
        if args.datatype == "mel":
            for predict_id in predict_list:
                # mel range from (-1,1) to (0,1)
                self.predict_acoustics[predict_id] = (np.load(os.path.join(predict_dir, predict_id)).T + 1.0) / 2.0
                self.target_acoustics[predict_id] = (np.load(os.path.join(target_dir, target_dict[predict_id])) + 1.0) / 2.0
                # self.predict_acoustics[predict_id] = np.load(os.path.join(predict_dir, predict_id)).T
    ...
    def __call__(self):
        ...
        for key in tqdm(self.predict_acoustics):
            # print(self.predict_acoustics[key].shape)
            # print(self.target_acoustics[key].shape)
...
if __name__ == '__main__':
    ...
    predict_acoustic_files = [fp for fp in os.listdir(args.predict_acoustic_dir) if fp.endswith('.npy')]
